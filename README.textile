...
h3. Validate an ActiveModel (or ActiveRecord) object

Since ActiveRecord classes include ActiveModel, this will also work for ActiveRecord objects.
...
TestModel.new({'required_property' => "defined"}).valid? # => true
TestModel.new({}).valid? # => false
</pre>
